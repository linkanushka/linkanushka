# Hi there, I'm Anushka! 👋

**Aspiring Software Developer | Cloud Computing Enthusiast | Full-Stack Developer**

Welcome to my GitHub profile! I'm a B.Tech Computer Science Engineering student with specialization in Cloud Computing & Virtualization, passionate about building scalable applications and exploring emerging technologies.

---

## 🚀 About Me

- 🎓 **B.Tech CSE Student** at Galgotias University (2022–2026) | CGPA: 8.36
- 💼 **Software Development Trainee** at Capgemini (completed 3-month training program)
- 🌐 **Full-Stack & Frontend Developer** with hands-on project experience
- ☁️ Specializing in **Cloud Computing & Virtualization**
- 💡 Passionate about problem-solving, clean code, and continuous learning

---

## 🛠️ Tech Stack

### **Languages**
![Java](https://img.shields.io/badge/Java-ED8B00?style=flat-square&logo=java&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![C++](https://img.shields.io/badge/C++-00599C?style=flat-square&logo=cplusplus&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-336791?style=flat-square&logo=postgresql&logoColor=white)

### **Frameworks & Libraries**
![Spring Boot](https://img.shields.io/badge/Spring_Boot-6DB33F?style=flat-square&logo=spring-boot&logoColor=white)
![React](https://img.shields.io/badge/React-61DAFB?style=flat-square&logo=react&logoColor=black)
![Thymeleaf](https://img.shields.io/badge/Thymeleaf-005C0F?style=flat-square&logo=thymeleaf&logoColor=white)
![HTML5](https://img.shields.io/badge/HTML5-E34C26?style=flat-square&logo=html5&logoColor=white)
![CSS3](https://img.shields.io/badge/CSS3-1572B6?style=flat-square&logo=css3&logoColor=white)
![Tailwind CSS](https://img.shields.io/badge/Tailwind_CSS-38B2AC?style=flat-square&logo=tailwind-css&logoColor=white)

### **ORM & Build Tools**
![Hibernate](https://img.shields.io/badge/Hibernate-59666C?style=flat-square&logo=hibernate&logoColor=white)
![JPA](https://img.shields.io/badge/JPA-6DB33F?style=flat-square&logo=java&logoColor=white)
![Maven](https://img.shields.io/badge/Maven-C71A36?style=flat-square&logo=apache-maven&logoColor=white)

### **Databases**
![MySQL](https://img.shields.io/badge/MySQL-4479A1?style=flat-square&logo=mysql&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-13AA52?style=flat-square&logo=mongodb&logoColor=white)

### **Cloud & DevOps**
![AWS](https://img.shields.io/badge/AWS-FF9900?style=flat-square&logo=amazon-aws&logoColor=white)
![Cloud Computing](https://img.shields.io/badge/Cloud_Computing-4285F4?style=flat-square&logo=google-cloud&logoColor=white)
![Virtualization](https://img.shields.io/badge/Virtualization-0078D4?style=flat-square&logo=hyper-v&logoColor=white)

### **Design & Tools**
![Figma](https://img.shields.io/badge/Figma-F24E1E?style=flat-square&logo=figma&logoColor=white)
![Adobe XD](https://img.shields.io/badge/Adobe_XD-470137?style=flat-square&logo=adobe-xd&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat-square&logo=github&logoColor=white)
![VS Code](https://img.shields.io/badge/VS_Code-007ACC?style=flat-square&logo=visual-studio-code&logoColor=white)
![IntelliJ IDEA](https://img.shields.io/badge/IntelliJ_IDEA-000000?style=flat-square&logo=intellij-idea&logoColor=white)

---

## 📊 Skills Proficiency

| Category | Proficiency | Skills |
|----------|-------------|--------|
| **Backend Development** | ████████░ 80% | Java, Spring Boot, Hibernate, JPA, Maven |
| **Frontend Development** | ███████░░ 70% | React, HTML5, CSS3, JavaScript, Tailwind CSS |
| **Database Management** | ███████░░ 75% | MySQL, MongoDB, SQL Queries |
| **Cloud & DevOps** | ██████░░░ 60% | AWS, Cloud Architecture, Virtualization |
| **UI/UX Design** | ██████░░░ 65% | Figma, Adobe XD, Responsive Design |
| **Version Control** | █████████░ 90% | Git, GitHub, Collaboration |
| **Problem Solving** | █████████░ 85% | DSA, Algorithm Optimization |

---

## 💼 Experience

### **Capgemini — Software Development Trainee**
*3 Months Training Program*
- 🎯 Gained practical exposure to software development concepts, debugging, and coding practices
- 💪 Strengthened knowledge in Java, Spring Boot, Hibernate, Maven, web technologies, and databases
- 🤝 Participated in technical assignments and collaborative learning activities
- ☁️ Learned cloud computing fundamentals and virtualization concepts

### **OTT Frontend Project — Frontend Developer**
*Ongoing Contribution*
- 🎨 Contributing to responsive UI components and layouts
- ⚡ Building modern frontend interfaces with HTML, CSS, and JavaScript
- 📱 Focusing on user experience and responsive design
- 🎭 Using Figma for UI/UX mockups and prototypes

---

## 📂 Featured Projects

### 🏥 **Hospital Management System**
**Tech Stack:** Spring Boot | Hibernate | JPA | MySQL | Maven | Thymeleaf | Git
- Complete patient record management and doctor management system
- Features: Appointment scheduling, secure authentication, CRUD operations
- Integrated backend database with Hibernate ORM for efficient query handling
- Built with Maven for dependency management and project build

**Repository:** [View Project](https://github.com/linkanushka/Hospital-Management-System)

---

### 📺 **Netmec OTT Platform Frontend**
**Tech Stack:** React.js | HTML5 | CSS3 | JavaScript | Figma
- Responsive frontend for OTT streaming platform
- Modern UI components with smooth navigation
- Designed with Figma before development
- Focus on user experience and responsive design

**Repository:** [View Project](https://github.com/linkanushka/netmec-ott)

---

### 📋 **Attendance Monitoring System**
**Tech Stack:** HTML | CSS | JavaScript | PHP | MySQL
- Secure login and attendance tracking system
- Dashboard with attendance summaries and report generation
- Full CRUD operations and database integration

**Repository:** [View Project](https://github.com/linkanushka)

---

## ☁️ Cloud Computing & Virtualization Expertise

- **AWS Services:** EC2, S3, Lambda basics
- **Cloud Concepts:** Infrastructure as a Service (IaaS), Platform as a Service (PaaS), Software as a Service (SaaS)
- **Virtualization:** Virtual machines, containerization concepts
- **Cloud Architecture:** Scalability, reliability, and deployment strategies

---

## 🌱 Currently Learning

- 🔄 Advanced React.js patterns and state management (Redux)
- ☁️ AWS services and cloud deployment (EC2, S3, RDS)
- 🐳 Docker and containerization
- 🤖 Introduction to AI/ML
- 📐 Advanced UI/UX Design principles

---

## 🎯 Interests & Goals

- 💻 Building scalable, production-ready applications using Spring Boot & Hibernate
- ☁️ Exploring advanced AWS services and cloud architecture
- 🎨 Creating intuitive and beautiful user interfaces with Figma
- 🚀 Deploying applications on cloud platforms
- 🧠 Learning AI/ML and emerging technologies
- 📚 Contributing to open-source projects
- 🔄 Continuous learning and skill development

---

## 🔗 Connect With Me

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0A66C2?style=flat-square&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/Anushka-Singh11?utm_source=chatgpt.com)
[![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat-square&logo=github&logoColor=white)](https://github.com/linkanushka)
[![Email](https://img.shields.io/badge/Email-D14836?style=flat-square&logo=gmail&logoColor=white)](mailto:your.email@example.com)

---

## 💭 Quote

> *"Code is poetry written for computers and humans alike. Every line tells a story of problem-solving and creativity. The cloud is where innovation meets scalability."*

---

### 📈 Fun Fact
I'm constantly exploring new technologies and love turning ideas into reality through code! When I'm not coding, you'll find me learning about cloud infrastructure, designing intuitive UX experiences, and contributing to tech communities.

**Let's collaborate and build something amazing together!** 🚀

---

*Last updated: May 2026*
